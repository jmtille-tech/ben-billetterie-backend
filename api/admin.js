...
async function supabaseRequest(path, options) {
  ...
  // Important : on fusionne les en-têtes par défaut AVANT de les combiner aux options,
  // sinon un appel qui passe ses propres headers (ex: Prefer) écrase apikey/Authorization
  // au lieu de s'y ajouter (Object.assign ne fusionne pas les objets imbriqués).
...
async function runScoring(entretienId) {
  ...
  // Shortlist de 5 : Jean-Marc choisit les 3 finaux lui-même, sur la base de son
  // expertise et de l'échange en rdv avec l'exploitant — le score ne fait que préparer.
...
// --- Notifications par email (validation d'un entretien) --------------------
// Utilise Resend (https://resend.com). Si RESEND_API_KEY n'est pas configurée,
// la notification est simplement ignorée (le reste de l'action 'validate'
// continue de fonctionner normalement) — utile tant que la clé n'est pas encore
// en place côté Vercel.
...
async function notifyValidation(entretien) {
  ...
  notifications.admin = await sendEmail(
    adminEmail,
    'BEN Billetterie — entretien validé : ' + nomSite,
    '<p>L\'entretien de <strong>' + (entretien.prenom || '') + ' ' + (entretien.nom || '') + '</strong> (' + nomSite + ') vient d\'être validé.</p>' +
    '<p><a href="' + ADMIN_URL + '">Ouvrir l\'espace interne</a> pour consulter le récapitulatif et calculer la shortlist d\'opérateurs.</p>'
  );
  ...
  if (entretien.email && CALENDLY_URL) {
    notifications.exploitant = await sendEmail(
      ...
      '<p>Merci pour le temps consacré à l\'entretien diagnostic BEN Billetterie. Votre récapitulatif est en cours d\'analyse.</p>' +
      '<p>Pour échanger sur les prochaines étapes, vous pouvez réserver un créneau directement ici : <a href="' + CALENDLY_URL + '">' + CALENDLY_URL + '</a></p>' +
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'validate') {
      ...
      // Notifications par email (best-effort : un échec ici ne fait pas échouer la validation).
      let notifications = null;
      try {
        const rows = await supabaseRequest(
          '/rest/v1/ben_entretiens?id=eq.' + encodeURIComponent(id) + '&select=prenom,nom,nom_site,email',
          { method: 'GET' }
        );
        if (rows && rows[0]) {
      ...
      res.status(200).json({ ok: true, notifications: notifications });
      return;
    }
